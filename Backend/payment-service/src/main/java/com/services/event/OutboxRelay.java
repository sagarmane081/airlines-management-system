 ...
 * exactly why the downstream consumers had to be made idempotent already.
 */
@Component
public class OutboxRelay {
    ...
    public OutboxRelay(OutboxEventRepository outboxEventRepository, PaymentEventProducer paymentEventProducer) {
    ...
    public void relayOne(OutboxEvent outboxEvent) {
        try {
            paymentEventProducer.publish(new PaymentCompletedEvent(
                            outboxEvent.getPaymentId(), outboxEvent.getBookingId(), outboxEvent.getAmount()))
